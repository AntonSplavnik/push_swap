...
#    Updated: 2024/11/27 22:40:12 by asplavni         ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
GNL_OBJ_DIR = $(OBJ_DIR)/shared/gnl
...
HEADER_ALGORITHM = $(INC_DIR)/algorithm.h
...
# Combine all shared source files
SHARED_SRC = $(GNL_SRC) $(INIT_SRC) $(OPERATIONS_SRC) $(PARSING_SRC) $(SORT_CHECK_SRC) $(UTILITY_SRC)
...
bonus: $(BONUS_OBJ) $(SHARED_OBJ)
	$(CC) $(CFLAGS) $(BONUS_OBJ) $(SHARED_OBJ) -o $(BONUS_TARGET)

# Rule to create object files (.o) from source files (.c)
$(OBJ_DIR)/%.o: $(SRC_DIR)/%.c $(HEADER_SHARED) $(HEADER_ALGORITHM) $(HEADER_BONUS) $(HEADER_GNL)
	mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -c $< -o $@

# Clean up object files only
clean:
	rm -f $(ALGO_OBJ) $(SHARED_OBJ) $(BONUS_OBJ)

# Full clean: removes object files and executables
fclean: clean
